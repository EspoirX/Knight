...
class RouterBuilder(internal val context: Context?, internal val path: String?) {
    ...
    companion object {
        ...
        internal const val KEY_COMPLETE_LISTENER = PKG + "CompleteListener"
        internal const val KEY_INTERCEPTOR = PKG + "Interceptor"
    }
    ...
    fun attachInterceptors(interceptors: MutableList<ISyInterceptor>) = apply {
        val list = keyMap[KEY_INTERCEPTOR] as MutableList<ISyInterceptor>?
        if (list == null) {
            keyMap[KEY_INTERCEPTOR] = interceptors
        } else {
            list.clear()
    ...
    fun addInterceptor(interceptor: ISyInterceptor) = apply {
        var list = keyMap[KEY_INTERCEPTOR] as MutableList<ISyInterceptor>?
        if (list == null) {
            list = mutableListOf()
            keyMap[KEY_INTERCEPTOR] = list
        }
